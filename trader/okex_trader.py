        ...
        for buy_order in self.buy_orders:
            ...
            if check_order:
                if check_order.get('status') == OrderStatus.CANCELED.value:
                    ...
                    # 买单成交，挂卖单.
                    logging.info(f"买单成交->价格: {check_order.get('price')}, 数量: {check_order.get('origQty')}")

                    sell_price = round_to(float(check_order.get("price")) * (1 + float(config.gap_percent)), config.min_price)
        ...
        for sell_order in self.sell_orders:
            ...
            if check_order:
                if check_order.get('status') == OrderStatus.CANCELED.value:
                    ...
                    logging.info(f"卖单成交->价格: {check_order.get('price')}, 数量: {check_order.get('origQty')}")
